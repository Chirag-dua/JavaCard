...
/// The I/O file is fragmented.
#define SDSC_IO_FILE_FRAGMENTED_ERROR	(SDSC_ERROR_BASE | 0x14)

/// The I/O file is incomplete.
#define SDSC_IO_FILE_INCOMPLETE_ERROR		(SDSC_ERROR_BASE | 0x15)

/// The received APDU response is invalid
#define SDSC_APDU_RESPONSE_ERROR			(SDSC_ERROR_BASE | 0x16)

/// The specified right of the device is incorrect, or the application has no permisson to do the operation.
#define SDSC_DEV_RIGHT_ERROR				(SDSC_ERROR_BASE | 0x25)

/// The device does not have sufficient space to perform the requested function.
#define SDSC_DEV_NO_SPACE_ERROR			(SDSC_ERROR_BASE | 0x2A)

/// The specified file in the device has not been created. 
#define SDSC_DEV_NO_FILE_ERROR			(SDSC_ERROR_BASE | 0x2C)
...
/**
 * The base error code for errors from system calls.
 * Whenever a function fails in a system call, it will return (SDSC_SYSTEM_ERROR_BASE | errno).
 */
#define SDSC_SYSTEM_CALL_ERROR_BASE			 0x0FE00000
