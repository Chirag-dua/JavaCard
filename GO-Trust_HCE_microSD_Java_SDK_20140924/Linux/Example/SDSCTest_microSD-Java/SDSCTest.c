...
int main()
{
	...
	// Atr
	unsigned char bAtr[32];
	unsigned long ulAtrLen;
    ...
    //init SDK
    printf("Please input I/O file name: ");
	...
	// set params, for example: /dev/sdb1,/media/disk
	pszParams = (char*)malloc(strlen("/dev/sdb1,/media/disk-1")+2);// "+2" for multi-string
	if(pszParams == NULL)
	{
		printf("malloc error.\n");
		goto err;
	}
	memset(pszParams, 0x00, strlen("/dev/sdb1,/media/disk-1")+2);
	strcpy(pszParams, "/dev/sdb1,/media/disk-1");
	...
	// connect dev
	if(ulRet=SDSCConnectDev(pszDrive, &hDevice, szIoFileName))
	{
	...
	// Reset 
	ulAtrLen = 32;
	ulRet = SDSCResetCard(hDevice,bAtr,&ulAtrLen);
	if(ulRet != 0)
	{
		printf("SDSCResetCard failure! Error code= 0x%08x.\n",ulRet);
		goto err;
	}
	printf ("ATR: ");
	for(i=0; i<ulAtrLen; i++)
		printf("%02x ",bAtr[i]);
	printf("\n");

	// Get SCIO type
	ulSCIOType = SDSC_SCIO_UNKNOWN;
	ulRet = SDSCGetSCIOType(hDevice, &ulSCIOType);
	if(ulRet != 0)
	{
		printf("SDSCGetSCIOType failure! Error code= 0x%08x.\n",ulRet);
		goto err;
	}
