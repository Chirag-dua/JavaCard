...
public class Command {
    ...
    public byte[] getPayload() {
        return payload;
        //return (new String(payload)).trim().getBytes();
    }
    ...
    public void addPayload(byte[] payload2) {
        byte[] p = Utils.ConcatArrays(this.payload, payload2);
        // Log.d(tag, "old Payload = "+new String(payload));
        Log.d(tag, "old Payload = "+Utils.ByteArrayToHexString(payload));
        Log.d(tag, "new Payload to add = "+Utils.ByteArrayToHexString(payload2));
        // Log.d(tag, "new Payload to add = "+new String(payload2));
        // Log.d(tag, "new Payload = "+new String(p));
        Log.d(tag, "new Payload = "+Utils.ByteArrayToHexString(p));
    ...
    public Command getNextCommandToSend() {
        ...
        Log.d(tag, "Next Command Payload: " + Utils.ByteArrayToHexString(cmd.getPayload()));
        return cmd;
    }

    public byte[] getCommandArray() {
        return Utils.ConcatArrays(new byte[] { cla }, new byte[] { ins },
                new byte[] { p1 }, new byte[] { p2 }, new byte[] { length },
                errorCheck, payload);
    }
}