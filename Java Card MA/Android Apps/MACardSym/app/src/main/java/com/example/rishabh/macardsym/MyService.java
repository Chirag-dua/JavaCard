    ...
    public Response onReceiveCommand(Command command)
    {
        ...
        switch (command.getIns())
        {
            case ((byte) 0x30):
                ...
                try
                {
                    len = obj.SDSCTransmitEx(phDevice, getRandsIdsFromSE, 0, getRandsIdsFromSE.length,
            ...
            case ((byte) 0x31):
                ...
                try {
                    ...
                    apduToBeSent = Utils.ConcatArrays(Arrays.copyOf(signC,len-2),
                            Arrays.copyOf(message,lenMod-2),
                            nextMessageToBeSent);
            ...
            case ((byte) 0x32):
                ...
                try
                {
                    len = obj.SDSCTransmitEx(phDevice, getMacsFromSE, 0, getMacsFromSE.length,
                            SESDAPI.SDSC_DEV_DEFAULT_TIME_OUT, bOutData);
                }
                catch(SDSCException e)
                {
